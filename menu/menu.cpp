...
namespace menu {
	...
	void init() {
		...
		if (isOpen)
		{
			ImGui::Begin("SWBF2", &isOpen, window_flags);
			if (ImGui::TreeNode("ESP")) 
			{
				ImGui::Checkbox("Enable ESP", &settings::ESP::enabled);
				ImGui::Checkbox("Show Enemy", &settings::ESP::enemy);
				ImGui::Checkbox("Show Name", &settings::ESP::name);
				ImGui::Checkbox("Show Health", &settings::ESP::health);
				ImGui::Checkbox("Show Dot", &settings::ESP::dot);
				ImGui::Checkbox("Show Ariel/Enforcer/Infiltrator (Orange/Yellow)", &settings::ESP::extraUnitCheck);
				ImGui::TreePop();
			}
			ImGui::Text("Press BACKSPACE to open/close menu.");
			ImGui::End();
		}
	}
}